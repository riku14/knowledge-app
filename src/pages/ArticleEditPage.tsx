...
import { getCategories, mockTags, getArticleById, updateArticle } from '../mockData'
...
export const ArticleEditPage = () => {
  ...
  useEffect(() => {
    ...
    setScope(article.scope || ArticleScope.PUBLIC)
    setTeamId(article.teamId || '')
    setContent(article.content)
    setIsLoading(false)
  }, [id])

  // 記事が見つからない、または権限がない場合
  if (!id) {
  ...
  if (article.author.id !== currentUserId) {
    return (
      <div className="container mx-auto px-4 py-6 max-w-7xl">
        <div className="text-center">
          <h1 className="mb-4 text-2xl font-bold text-gray-900">編集権限がありません</h1>
          <p className="text-gray-600">この記事を編集する権限がありません。</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">記事編集</h1>
        <button
